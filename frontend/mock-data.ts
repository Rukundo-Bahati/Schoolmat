...
export const allProducts: Product[] = [
  ...
  },
  {
    ...
    name: "Ruler Set",
    price: "1,500",
    image: "/placeholder.jpg?height=250&width=250",
    required: true,
    rating: 4,
    reviews: 189,
    inStock: true,
...
// Revenue data by term - calculated from delivered orders
export const salesDataByTerm = {
  labels: ["Term 1 (Sep-Dec)", "Term 2 (Jan-Mar)", "Term 3 (Apr-Jun)"],
  datasets: [
    {
      label: "Revenue (RWF)",
      data: [
        // Term 1: Sep-Dec - calculate from delivered orders in this period
        schoolOrders
          .filter(order => order.status === "delivered")
          .filter(order => {
            const month = new Date(order.orderDate).getMonth() + 1; // getMonth() returns 0-11
            return month >= 9 || month <= 12; // Sep=9, Oct=10, Nov=11, Dec=12
          })
          .reduce((sum, order) => sum + order.totalAmount, 0),
        // Term 2: Jan-Mar - calculate from delivered orders in this period
        schoolOrders
          .filter(order => order.status === "delivered")
          .filter(order => {
            const month = new Date(order.orderDate).getMonth() + 1;
            return month >= 1 && month <= 3; // Jan=1, Feb=2, Mar=3
          })
          .reduce((sum, order) => sum + order.totalAmount, 0),
        // Term 3: Apr-Jun - calculate from delivered orders in this period
        schoolOrders
          .filter(order => order.status === "delivered")
          .filter(order => {
            const month = new Date(order.orderDate).getMonth() + 1;
            return month >= 4 && month <= 6; // Apr=4, May=5, Jun=6
          })
          .reduce((sum, order) => sum + order.totalAmount, 0),
      ],
      backgroundColor: "rgba(59, 130, 246, 0.8)",
...
export const productPurchasesByTermData = {
  labels: ["Term 1 (Sep-Dec)", "Term 2 (Jan-Mar)", "Term 3 (Apr-Jun)"],
  datasets: [
    {
